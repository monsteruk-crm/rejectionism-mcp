...
- *Open unauthenticated access without an environment gate*: Rejected due to high risk of unintended data leakage or unauthorized mutations in deployed environments.
- *Full authentication (Clerk / NextAuth) in MVP*: Rejected per product specifications (MVP explicitly excludes authentication frameworks).
